...
#pragma warning disable CA2213 // Not disposing `_inputs`; instead, we dispose the underlying resources.
            ...
            public void SetContext(IOperatorContext context)
            {
                ...
                if (_observer is ISubscription sub)
                {
                    _inputs.Add(sub);
                }
                else if (_observer is IOperator op)
                {
                    _inputs.AddRange(op.Inputs);
                    op.SetContext(context);
                }
            }

            public void Start() => (_observer as IOperator)?.Start();
            ...
            public void Dispose()
            {
                ...
                (_observer as ISubscription)?.Dispose();
                (_observer as IOperator)?.Dispose();
            }
